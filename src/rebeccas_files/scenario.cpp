...
/* PPL ADDENDUM -- KEY FUNCTIONS FOR LOWER BOUNDING TASK */
#include <iostream>
#include <limits>
using namespace std;
...
void Scenarios::NewIndepDiscreteEntry_JLBV( const char *Row, const char *Col, // )
	const char *, double Value, double Prob )
{
	Print( "\nINSIDE NewIndepDiscreteEntry ..." );
	//--------------------------------------------------------------------------
	//	If this is the beginning of a new section, make sure the last block
	//	is stored on the distribution list.
	//
	...
	{
		if (len > 0)
			for (int i = 0; i < MAX_NODES; i++){    // Note Max_Nodes is the maximum of leaf nodes in the sadsam tree at any time, it should be set 
		                                            // equal to the total number of scenarios for the problem at hand: define as a const in scenario
                                                    // file.
				IndepStochVar *isv_JLBV = new IndepStochVar( 0.0, LastIndepDiscrete.type, LastIndepDiscrete.row, LastIndepDiscrete.col, 0.0 );
				if( isv_JLBV == NULL ) FatalError( "Out of memory." );	

				dist[len-1]->Append( isv_JLBV );
			}		
...
void Scenarios::EndData_JLBV( void )
{
	switch( LastStochBlockType )
	{
	case INDEP_DISCRETE:
...
/* RS addendum */
Real_T Scenario::GetValue(const Scenario &s1)
{
	Real_T value = 0.0;
	...
	{		
		for( Int_T j = 0, bl = s1[i].Len(); j < bl; j++ )
		{
			...
			// this uses a weighted norm for APL1P, and an unweighted norm for all other test problems
#if defined apl1p_opt 
			if(i == 4 || i ==5)
			{
				value += pow( 1000*d1.value, 2 );
			}
			else
			{
				value += pow( d1.value, 2 );
			} 
						  
#elif defined apl1p_sub
			if(i == 4 || i ==5)
			{
				value += pow( 1000*d1.value, 2 );
			}
...
Real_T Scenario::GetDistance( const Scenario &s1, const Scenario &s2 )
{
	...
	case EUCLIDEAN:     	return GetEuclideanDistance( s1, s2 );
	default:
#ifndef NDEBUG
		abort();
#endif
...
Real_T Scenario::GetEuclideanDistance( const Scenario &s1, const Scenario &s2 )
{
	assert( DistMode == EUCLIDEAN );
	assert( s1.GetLength() == s2.GetLength() );
...
// this uses a weighted norm for APL1P, and an unweighted norm for all other test problems
			...
			if(i == 4 || i ==5)
			{
				norm += pow( 1000*(d1.value - d2.value), 2 );
			}
			else
			{
				norm += pow( d1.value - d2.value, 2 );
			} 
...
Real_T Scenario::GetWeightedNormDistance( const Scenario &s1, const Scenario &s2 )
{
...
Real_T Scenarios::GetDistance( Int_T s1, Int_T s2 )
{
	...
	//Scenario::SetDistanceMode(Scenario::DistanceMode(EUCLIDEAN));    // PPL ADDENDUM
...
Real_T Scenarios::GetValue(Int_T s1)
{
	return Scenario::GetValue( *ArrayOfScenarios[s1] );
...
Int_T Scenarios::Dimension(void)         // returns the dimension of the random data
{
	return len;
}


Int_T Scenarios::SampleSize(Int_T i)                   // returns the sample size of component 'i'
{
	return dist[i]->Len() - MAX_NODES;

}
    
Real_T Scenarios::Value(Int_T i, Int_T j)              // returns the value of jth realization of the ith component
{
    return (*dist[i])[j][0].value;

}


Real_T Scenarios::Prob(Int_T i, Int_T j)               // returns the probability of the jth realization of the ith component 
{
	return (*dist[i])[j].GetProbability();
}

ListNode::ListNode(ListNode *SomeNode, const int dim)  // Copy constructor: used when partitioning to add a new node
{
	k     = SomeNode->k;
	level = SomeNode->level;
	ScenPos = SomeNode->ScenPos;
	n_k = SomeNode->n_k;
	p_k = SomeNode->p_k;
	flag = SomeNode->flag;
	LB = SomeNode->LB;
	UB = SomeNode->UB;
	UB_std = SomeNode->UB_std;
	...
	for (int i=0; i < dim; i++){
		Component[i].a_k = SomeNode->Component[i].a_k;
		Component[i].b_k = SomeNode->Component[i].b_k;
		Component[i].ExpectedValue = SomeNode->Component[i].ExpectedValue;

		Component[i].Cum_Prob.resize(SomeNode->Component[i].distr.size());
		Component[i].Prob.resize(SomeNode->Component[i].distr.size());
		Component[i].distr.resize(SomeNode->Component[i].distr.size());

		for (int j=0; j < SomeNode->Component[i].distr.size(); j++){
			Component[i].Cum_Prob[j] = SomeNode->Component[i].Cum_Prob[j];
			Component[i].distr[j] = SomeNode->Component[i].distr[j];
			Component[i].Prob[j] = SomeNode->Component[i].Prob[j];
		}
        
		/* The part below is not needed for now -- its implementation will depend on how we implement the upper bounding tasks */
		/*for (int j=0; j < SomeNode->Component[i].sample.size(); j++){
			Component[i].sample[j] = SomeNode->Component[i].sample[j];
		}*/

		Taylor[i].grad.resize(dim);
		Taylor[i].flag = 0;

	}
	Taylor[dim].grad.resize(dim);
	Taylor[dim].flag = 0;

}

void ListNode::UpperBound(double x_hat[])               // Calculates upper bound contribution of that node; updates UB
{  	                                                    // This might be tough to do, but we'll see. 
; /* To write later */                                  // May be good to have a similar function for LB as well.
}

ListNode::ListNode(void)                                // Default Constructor for ListNode
{   
	k = 1; 
	level = 0;
	ScenPos = 0;
	p_k = 1;
	flag = 1;
	LB = -numeric_limits<double>::infinity( );          // Some default very small value... -Inf
	UB = numeric_limits<double>::infinity( );           // Some default very large value... +Inf 
}


void Scenarios::ScenCopy(int pos, Scenario& SomeScen)              // points ArrayOfScenarios[pos] to SomeScen
{
	ArrayOfScenarios[pos][0] = SomeScen; 
}

Scenario& Scenarios::GetScen(int pos)                     // returns address of scenario pos, i.e., ArrayOfScenario[pos][0]
{
	return ArrayOfScenarios[pos][0];
}


void Scenarios::Update(int pos, double prob, vector <ComponentStruct> Component)            // updates the scenario repository corresponding to the leftmost of the split nodes
{
	ArrayOfScenarios[pos]->SetProbability(prob);
    
	//cout<<"INSIDE function Update() -- node "<<pos<<" prob = "<<prob<<endl;
	//cout<<"INSIDE function Update() -- Probability is Set = "<<ArrayOfScenarios[pos]->GetProbability()<<endl; // getchar(); // runtime test checkpoint
	
	for (int i=0; i < ArrayOfScenarios[pos]->GetLength(); i++ ){
		//cout<<"Currently Stored Value of Component "<<i+1<<" : "<<ArrayOfScenarios[pos][0][i][0].value<<" --- ";
		//cout<<" Expected Value = "<<Component[i].ExpectedValue<<endl;
		
		ArrayOfScenarios[pos][0][i][0].value = Component[i].ExpectedValue; // need to do heavy testing to make sure this is correct
...
void Scenarios::AppendAndUpdate(int pos, double prob, vector <ComponentStruct> Component, int k)   // appends a new scenario to the scenario repository correspoding to the rightmost of the split nodes for LB only
{
	int NumAppend, InitScenNum = ScenNum;
		
	NumAppend = k - InitScenNum;  
	...
	if( NumAppend >0){
		...
		if( k > MAX_SCEN_NUM )
		{
			Print( "\tSpecified number of scenarios exceeds the maximum number of scenarios"
				"allowed.\n\tAdjusting to %d.\n",
				int( k = (Int_T) MAX_SCEN_NUM ) );
		}
		
		
		assert( k > 0 && k <= MAX_SCEN_NUM );
		...
		//Loop on all scenarios to be appended
		for( int s = 0; s < NumAppend; s++ )
		{
			//create a new scenario; resize the array and add the new scenario to the array
			Scenario *scen_a = new Scenario( len, prob);
			if( !scen_a ) FatalError( "Out of memory." );
			...
			for( int i = 0; i < len; i++ )
			{
				...
				//cout<<"Inside AppendAndUpdate(): Assigning location for  component "<<i<<": "<<endl; // getchar(); // runtime test
			
				//cout<<"Component Expected Value =  "<<Component[i].ExpectedValue<<endl; // getchar();

				//cout<<" All currently stored values for component "<<i<<" :";
				//for( int j2 = 0; j2 < k; j2++ )
		        //   cout<<"  "<<(*dist[i])[j2][0].value;
	            //cout<<endl ; // getchar();

				//cout<<"Setting scen_a"<<endl;
			
				scen_a->Set( i, &( (*dist[i])[pos] ) );
			}
		}
        
		Update(pos, prob, Component);
...
/* ----------------------------------------------------------------------------------------------------------------   */
/* ----------------------------------------------------------------------------------------------------------------   */
void Scenarios::ReGenerateScenarios_TEV( ListNode *node, int pos_bk )
{
	//cout<<"Inside Regenerate Scenarios TEV "<<endl; // getchar(); // runtime test checkpoint
	...
	for(int s = 0; s < ScenNum; s++ ){
		for(int i = 0; i < len; i++ ){
			if (i == pos_bk - 1){
				//cout<<"Adding Element b["<<i<<"] For Scenario "<<s<<" and node "<<node->ScenPos<<" : "<<node->Component[i].b_k<<endl; // getchar(); // runtime test checkpoint
			    ArrayOfScenarios[s]->SetAgain( i, &( (*dist[i])[node->Component[i].b_k] ) );
			}
			else{
				//cout<<"Adding Element a["<<i<<"] For Scenario "<<s<<" and node "<<node->ScenPos<<" : "<<node->Component[i].a_k<<endl; // getchar(); // runtime test checkpoint
				ArrayOfScenarios[s]->SetAgain( i, &( (*dist[i])[node->Component[i].a_k] ) );
			}
		}
	}
		
	//cout<<"All TEV Scenario regenerated"<<endl; // getchar(); // runtime test checkpoint
}  
...
/* ----------------------------------------------------------------------------------------------------------------   */
/* ----------------------------------------------------------------------------------------------------------------   */
void Scenarios::ReGenerateScenarios_UBV( ListNode *node )
{
	//cout<<"Inside Regenerate Scenarios "<<endl; // getchar(); // runtime test checkpoint

	assert( node->n_k > 0 && node->n_k <= MAX_SCEN_NUM );

	//--------------------------------------------------------------------------
	//	If needed, resize. Also, Re-assign the scenario probabilities.
	//
	if (node->n_k > ScenNum)
		AppendScenario( node->n_k );

	if( node->n_k != ScenNum ){
		ScenNum = node->n_k; 
		this ->ArrayOfScenarios.Resize( ScenNum );
		//cout<<"ScenNum adjusted to "<<ScenNum<<endl; // getchar(); // runtime test checkpoint
	}
	...
	//	Loop on all scenarios. Then, loop on distributions and re-form scenarios
	//
	for(int s = 0; s < node->n_k; s++ ){
		for(int i = 0; i < len; i++ ){
			//cout<<"Adding Element "<<i<<" For Scenario "<<s<<" : "<<node->Component[i].sample[s]<<endl; // getchar(); // runtime test checkpoint
			ArrayOfScenarios[s]->SetAgain( i, &( (*dist[i])[node->Component[i].sample[s]] ) );
		}
	}
		
	
	//cout<<"All Scenarios Regenerated"<<endl; // getchar(); // runtime test checkpoint
}  
...
Bool_T Scenarios::GenerateScenarios_JLBV() /* this generates one single scenario which corresponds to the mins of each random component. 
									      It is called at the start of the program within ReadAndGenerateScenarios_JLBV. Only use with 
										  Scen_LB scenario repository to perform lower bound with SADSAM. NOT GOOD FOR ANY OTHER USE! */
{
	Bool_T GenerateAll = False;
	...
	//Print( "\tGenerating 1 scenario now...\n");

	ArrayOfScenarios.Resize(1);
	ArrayOfScenarios.Fill( NULL, 1);

	Scenario *scen = new Scenario( len, 1.0);

	if( !scen ) FatalError( "Out of memory." );
	ArrayOfScenarios[0] = scen;
...
#ifndef NDEBUG
	{
		assert( ArrayOfScenarios[0] != NULL );
	}
#endif
...
Bool_T Scenarios::ReadScenarioFile_JLBV( const char *StochFileName, FILE *StochFilePtr, DeterministicLP &DeterministicMatrix )
{
	status = READING_FILE;
	...
	//	Read in the stoch file.
	//
	if( !GetStochFile_JLBV( StochFileName, StochFilePtr, this ) || SemanticError )
	{
#ifndef NDEBUG
		CheckArrayOfDistributions();
#endif
